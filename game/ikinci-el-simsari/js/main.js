...
function render(){
  HeaderView.update();
  var main=document.getElementById('main'); main.innerHTML=''; main.classList.remove('fadein'); void main.offsetWidth; main.classList.add('fadein');
  var tab=Game.state.tab;
  if(tab==='listings') ListingsView.render(main); else if(tab==='garage') GarageView.render(main); else if(tab==='mesajlar') MesajlarView.render(main); else if(tab==='panel') ControlPanelView.render(main); else if(tab==='ustalar') UstalarView.render(main); else if(tab==='dukkanlar') DukkanlarView.render(main); else if(tab==='profil') ProfilView.render(main); else if(tab==='leaderboard') LeaderboardView.render(main); else if(tab==='harita') MapView.render(main);
  var activeNavTab=PANEL_CHILD_TABS.indexOf(tab)>=0?'panel':tab;
  document.querySelectorAll('#bottombar button').forEach(function(b){b.classList.toggle('active',b.dataset.tab===activeNavTab)});
  var existingOverlay=document.querySelector('.overlay'); if(existingOverlay) existingOverlay.remove();
