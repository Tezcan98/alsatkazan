...
var client = configured ? createClient(SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY, {auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}}) : null;

export var AuthService = {
  client:client, configured:configured, user:null,
  init:async function(){
    if(!client)return null;
    var self=this, r=await client.auth.getSession();
  ...
  signUp:async function(email,password,name){
    ...
    var r=await client.auth.signUp({email:email.trim().toLowerCase(),password:password,options:{emailRedirectTo:window.location.origin+window.location.pathname,data:{display_name:(name||'Simsar').trim()}}});
  ...
  resetPassword:async function(email){
    if(!client)throw new Error('Üyelik sistemi yapılandırılmamış.');
    var r=await client.auth.resetPasswordForEmail(email.trim().toLowerCase(),{redirectTo:window.location.origin+window.location.pathname});if(r.error)throw r.error;return r.data;
  }
};
