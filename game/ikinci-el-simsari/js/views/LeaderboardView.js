...
export var LeaderboardView = {
  render: function(container){
    var p=Game.player, score=Game.score();
    var rivals=[
    ...
    var rows=rivals.concat([{name:p.name,score:score,tag:'Oyuncu',you:true}]).sort(function(a,b){return b.score-a.score;});
    var rank=rows.findIndex(function(x){return x.you;})+1;

    var hero=document.createElement('div');
    hero.className='league-hero';
    hero.innerHTML='<div class="league-orb"><img src="'+IMG.icon.trophy+'" alt=""></div><div><div class="kicker">PERFORMANS</div><h1>Skor Tablosu</h1><p>Toplam skorun <b>'+score.toLocaleString('tr-TR')+'</b> · Sıra #'+rank+'</p></div>';
    container.appendChild(hero);

    var grid=document.createElement('div'); grid.className='stat-grid';
    [['Skor',score.toLocaleString('tr-TR')],['Satış',p.totalSales],['Toplam kâr',fmt(p.totalProfit)],['En iyi seri',p.bestDealStreak]]
      .forEach(function(x){var c=document.createElement('div');c.className='stat-card';c.innerHTML='<div class="kicker">'+x[0]+'</div><strong>'+x[1]+'</strong>';grid.appendChild(c);});
    container.appendChild(grid);

    var h=document.createElement('h2');h.className='section';h.textContent='Günlük Hedefler';container.appendChild(h);
    ...
    note.textContent='Skor; kâr, satış, yetenekler, başarımlar, varlık değeri, XP ve satış serisine göre hesaplanır.';
